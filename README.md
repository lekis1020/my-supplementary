...
- **원료 중심 구조** — 672종 공개 성분(12개 타입 분류)에 대한 기능성, 근거 등급, 안전성 정보(2,000건+)
- **식약처 전수 수집** — 신고 제품 44,000건+ 수집, 이 중 **실제 판매가 확인된 4,600건+만 기본 노출** (신고만 있는 제품은 제품명 직접 검색으로만 접근)
- **실시간 검색 폴백** — DB에 판매확인 제품이 부족한 검색어는 Naver Shopping 실시간 검색으로 보강, 유입 제품은 비공개 적재 후 식약처 매칭 성공 시 공개
- **과학적 근거 기반** — PubMed 논문의 정량 데이터(효과크기, p-value, 신뢰구간) 수록
- **규제 데이터 연동** — 한국 식약처 + 미국 DSLD/DailyMed 라벨 데이터
- **제품 비교** — 성분 함량, 근거 수준, 가격 대비 함량 비교 (최대 4개)
...
PubMed 체계적 문헌고찰/메타분석 논문에서 추출한 정량 근거 데이터:
...
## Current Scope (2026-07 기준)

MVP(원료 20종 + 제품 30~50개)를 넘어 **식약처 신고 제품 전수 수집 체제**로 확장되었습니다.

| 항목 | 규모 |
|------|------|
| 공개 성분 | 672종 (12개 타입) |
| 수집 제품 | 44,000건+ (식약처 신고 전수) |
| 판매 확인 제품 | 4,600건+ (기본 노출 대상) |
| 제품 이미지 | 5,200건+ (Gemini Vision 검증, R2 적재) |
| 안전성 항목 | 2,000건+ |
| 기능성 클레임 | 면역, 골건강, 수면, 심혈관, 항산화, 장건강, 눈건강 등 23종 분류 |

상세 현황과 노출 정책은 [PRD v2](docs/PRD.md)를 참조하세요.
