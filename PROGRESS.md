...
- 통합 마이그레이션 파일 (`db/RUN_THIS_ONLY.sql` — frozen 2026-08-18, see `db/README.md`)
...
### Day 8 — 2026-04-12: 비타민 카테고리 데이터 정비 (Claude, 사용자 협업)

원격 Supabase 프로젝트 `loqhpykkovwczdckekju` 에 SQL Editor로 직접 적용 (Supabase Pooler 불안정으로 CLI `db push` 경로 포기).

**생성·적용된 마이그레이션 5개**
- `20260412120000_vitamin_completion.sql` — 비타민 B1/B2/B3/B5/K 신규 추가, biotin 카테고리를 `other` → `vitamin` 수정, 동의어 53건
- `20260412130000_vitamin_synonyms_restore.sql` — 기존 8종(A/C/D/E/B6/B12/folate/biotin) 동의어 78건 복원 (005 seed 미적용 또는 정리된 상태 복구)
- `20260412140000_vitamin_d_dosage_fix.sql` — 비타민 D 한국 2020 DRI 정합 (성인 400 IU / 65세+ 600 IU / 임산부 400 IU / 영아 400 IU, UL 4,000 IU)
- `20260412150000_cleanup_kr_prefixes_and_dosage_dedup.sql` — `safety_items.title/description` 앞의 `(가)/(나)/(다)/(라)/(마)` 공전 항목 번호 전수 제거
- `20260412160000_merge_iron_duplicates.sql` — `철`(id=510, slug=NULL, KR 임포트 유래)을 `철분`(id=8, slug=iron)으로 병합. 통합 후 2,554 product mapping

**발견된 데이터 품질 이슈 (다음 세션 후보)**
- 복합 비타민 태그 37건(`canonical_name_ko LIKE '비타민/%'`) 전부 `product_usage=0` — KR 식약처 카테고리 코드 노이즈
- `product_ingredients.amount_per_serving` 커버리지 49.8%, `daily_amount` 0.1% (함량 임포트 재검토 필요)
- 비타민 safety_items 0건: C, E, B6, folate
- 미네랄 카테고리 전수 점검 미수행 (철 외 다른 중복 가능성)

**도구적 교훈**
- Supabase SQL Editor는 BEGIN/COMMIT 블록을 개별 커밋하지 않는 경우 있음 → 개별 문장으로 실행
- Pooler 연속 인증 실패 시 circuit breaker 발동 (~20분 차단)
- `ingredients.canonical_name_ko`의 유니크 제약 존재 — slug UPSERT가 이를 못 잡음 → `ON CONFLICT DO NOTHING` + 사전 UPDATE 정규화 패턴
