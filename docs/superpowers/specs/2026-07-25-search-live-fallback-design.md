...
- fetch 로직, 로딩 스켈레톤, "실시간 검색 결과" 배지는 기존 그대로.
- 카드는 클릭 불가능한 순수 정보 카드로 변경한다 — 썸네일(있으면) +
  제품명 + 브랜드만 표시. 가격(lprice), 몰 이름(mallName), 외부 구매
  링크(`<a>` + ExternalLink 아이콘)는 모두 제거. 쇼핑 중개로 보이지
  않도록 구매 경로를 완전히 배제한다.
- 출처는 카드가 아닌 섹션 수준에서 표기한다 — 배지 옆 안내 문구에
  "네이버 쇼핑 검색 결과" 출처를 명시.
- 공유 컴포넌트이므로 성분 상세 페이지 폴백 카드에도 동일 적용.
...
- 실시간 카드에는 제품명 / 브랜드만 표시 (가격·몰·구매 링크 미표시,
  클릭 불가). 출처는 섹션 안내 문구의 "네이버 쇼핑 검색 결과"로만 표기.
